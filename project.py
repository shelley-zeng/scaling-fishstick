...
class Window:
    ...
    def __init__(self, window):
        """This init function has the grid layout and also the things for the start page
        ...
        self.butt = Button(window, text = "START", command=lambda: [self.welcome.destroy(), self.butt.destroy(),self.next_button(window)])
        ...
        self.q_a = {"\nWhat colours do you like?":["Black/White"," Neutral","Pastel","Bold"],
                    "What does a majority of your day \nconsist of?":["Walking","Sitting","Running","Ziplining"],
                    "What is something important when \nyou buy shoes?":["Comfort","Easy to run","They look cool","Gives me height"],
    ...
    def next_button(self, window):
        """This makes the user go to the next question
        """
        user_retry =0

        # FROM 2nd q. up only if the user has entered something in their previous question
        if 0<self.question_number < len(self.answer_list):
            ...
            self.btn_1 = Button(window, text = self.the_answers[0], command = lambda:self.results(1))
            self.btn_1.grid(row =1, column =1, columnspan =2)
            self.btn_2 = Button(window, text = self.the_answers[1], command = lambda:self.results(2))
            self.btn_2.grid(row =1, column =3, columnspan =2)
            self.btn_3 = Button(window, text = self.the_answers[2], command = lambda:self.results(3))
            self.btn_3.grid(row =2, column =1, columnspan =2)
            self.btn_4 = Button(window, text = self.the_answers[3], command = lambda:self.results(4), )
            self.btn_4.grid(row =2, column =3, columnspan =2)

            # questions
            self.question_frame = tk.Frame(master=window, bg = "#ECECEC", width = 200, height = 50)
            self.question_label = tk.Label(self.question_frame, text = self.question_list[self.question_number],bg = "#ECECEC", fg= "#609F63")
            self.question_frame.grid(row=0, column = 1, columnspan = 4)
            self.question_frame.grid_propagate(False)
            self.question_label.grid()

            self.nxt_button = Button(window, text = ">", command = lambda: [self.save_answer(window),self.next_button(window)], width = 3)
        ...
        # this is for the first question
            ...
            self.bck_button = Button(window, text = "<", command = lambda: [self.back_question(),self.back_button(window)], width = 3)
            self.bck_button.grid(row =3, column =2)
            self.nxt_button = Button(window, text = ">", command = lambda: [self.save_answer(window),self.next_button(window)], width = 3)
            self.nxt_button.grid(row =3, column =3)

            # this adds the question number into a list
            self.numb_lbl = tk.Label(window, text = f"{self.question_number+1}/{len(self.question_list)}")
            self.numb_lbl.grid(row =3, column =0)
            user_retry +=1
    ...
    def results(self, num):
        # must change append so that when user goes back the question thats meant to be changed is changed
    ...
    def no_answer(self, window):
        # if the user gives no answer a pop up will appear
        pop_up = Toplevel(window)
        pop_up.geometry("170x80")
        pop_up.resizable(False,False)
        error_label = tk.Label(pop_up, text = "Please select an option before \ncontinuing")
        error_label.grid()
        close_butt = Button(pop_up, text = "CLOSE", command = lambda:pop_up.destroy())
        close_butt.grid()
    ...
    def saved_from_back(self):
        if len(self.users_answer_list) == 1:
            self.result_list.insert(self.question_number, self.users_answer_list[0])
            self.add_question()
    ...
    def back_button (self,window):
        ...
        if 0 <=self.question_number < len(self.answer_list):
            ...
            self.btn_1 = Button(window, text = self.the_answers[0], command = lambda:self.results_conjugate(1))
            self.btn_1.grid(row =1, column =1, columnspan =2)
            self.btn_2 = Button(window, text = self.the_answers[1], command = lambda:self.results_conjugate(2))
            self.btn_2.grid(row =1, column =3, columnspan =2)
            self.btn_3 = Button(window, text = self.the_answers[2], command = lambda:self.results_conjugate(3))
            self.btn_3.grid(row =2, column =1, columnspan =2)
            self.btn_4 = Button(window, text = self.the_answers[3], command = lambda:self.results_conjugate(4))
            self.btn_4.grid(row =2, column =3, columnspan =2)

            self.question_frame = tk.Label(self.question_frame, text = self.question_list[self.question_number],bg = "#ECECEC", fg= "#609F63")
            self.question_frame.grid(row=0, column = 1, columnspan = 4)
            ...
            self.nxt_button = Button(window, text = "RR", command = lambda: [self.saved_from_back(),self.next_button(window)], width = 3)
            self.nxt_button.grid(row =3, column =3)
