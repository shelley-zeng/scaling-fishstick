...
class Window:
    ...
    def __init__(self, window):
        ...
        self.butt = Button(window, text = "START", command=lambda: [self.go(window), self.butt.destroy(),self.welcome.destroy()])
    ...
    def go(self,window):
        ...
        self.fing_frame = tk.Frame(master=window, bg = "#ECECEC", width = 200, height = 50)
        self.lb = tk.Label(self.fing_frame, text = self.q[self.question_number],bg = "#ECECEC", fg= "#609F63")
        self.fing_frame.grid(row=0, column = 1, columnspan = 4)
        self.fing_frame.grid_propagate(False)
        self.lb.grid(padx = 25)
        ...
        self.btn_1 = Button(window, text = self.needed_boop[0], command = lambda:self.results(1, self.result_list))
        self.btn_1.grid(row =1, column =1, columnspan =2)
        self.btn_2 = Button(window, text = self.needed_boop[1], command = lambda:self.results(2, self.result_list))
        self.btn_2.grid(row =1, column =3, columnspan =2)
        self.btn_3 = Button(window, text = self.needed_boop[2], command = lambda:self.results(3, self.result_list))
        self.btn_3.grid(row =2, column =1, columnspan =2)
        self.btn_4 = Button(window, text = self.needed_boop[3], command = lambda:self.results(4, self.result_list))
        self.btn_4.grid(row =2, column =3, columnspan =2)

        self.bck_button = Button(window, text = "<", command = lambda: [self.back_button(window)],width = 3)
        self.bck_button.grid(row =3, column =2)
        self.nxt_button = Button(window, text = ">", command = lambda: [self.next_button(window)],width = 3)
    ...
    def next_button(self, window):
        ...
        if self.question_number < len(self.boop) :
            ...
            self.finish_btn = Button(window, text = ">", command = lambda: self.the_end(window), width = 3)
